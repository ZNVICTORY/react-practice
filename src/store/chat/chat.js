...
export function chat(state=defaultState, action) {
  ...
  switch(action.type) {
    ...
    case MSG_READ: 
      ...
      ;return {  ...state, 
      chatmsg: state.chatmsg.map(v => ({...v, read: from === v.from? true: v.read})), unread: (state.unread - num)};
...
function msgRead({userid, from ,num}) {
  return {
    type: MSG_READ,
    payload:  { userid, from, num }
  }
}
export function readMsg(from) {
  return (dispatch, getState) => {
    axios.post('/api/v1/user/readmsg', {from}).then(res => {
      if(res.status === 200 && res.data.code === 0) {
        const userid = getState().user._id
        console.log('radasmag',res.data)
